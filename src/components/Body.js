...
const Body = () => {
  ...
  return listOfRestaurants.length===0? (
    ...
    <div>
     ...
     <div className='container flex h-8 '>
      
      <div className='flex '  >
      <input type='text' className='flex border rounded-lg text-xl w-40 border-black bg-slate-50 p-2  ml-5 hover:bg-slate-100' value={searchText} onChange={(e)=>{
            setSearchText(e.target.value);
        }}/>
        <button className='flex border w-16 rounded-xl border-black bg-slate-50 hover:bg-slate-100'
          onClick={()=>{
        ...
        > <h4 className='ml-2'>Search </h4></button>
      </div>
        
      
      <button className={filter1col?"flex border rounded-xl border-black px-1 ml-4 bg-slate-300":"flex border rounded-xl border-black px-1 ml-4 bg-slate-50   hover:bg-slate-100"}
        onClick={()=>{    
          if(!filter1col){
            const filteredList=listOfRestaurants.filter(
      ...
      >
        ...
        <button className= {filter2col?"flex border rounded-xl border-black px-1 ml-4 bg-slate-300":"flex border rounded-xl border-black px-1 ml-4 bg-slate-50   hover:bg-slate-200"}
        onClick={()=>{
